...
async function routerGet(cmd: string) {
  ...
  const response = await fetch(
    `${baseUrl}/goform/goform_get_cmd_process?${query}`,
    {
...
async function login() {
  if (!username || !password)
    throw new Error("Router credentials are not configured");
  ...
  const response = await fetch(`${baseUrl}/goform/goform_set_cmd_process`, {
    method: "POST",
    headers: {
      ...routerHeaders,
      Origin: baseUrl,
...
async function accessId() {
  ...
  return md5(
    `${md5(`${String(language.wa_inner_version || "")}${String(language.cr_version || "")}`)}${String(random.RD || "")}`,
  );
}
...
function requireConfirmed(value: unknown) {
  if (value !== true) throw new Error("Confirmation is required");
}
...
export async function POST(request: Request) {
  try {
    ...
    if (action === "wifi_update") {
      ...
      const passphrase =
        suppliedPassword ||
        decodeURIComponent(escape(atob(String(current.WPAPSK1_encode || ""))));
      ...
      const maxDevices = Math.max(
        ...
        Math.min(
          32,
          Number(payload.maxDevices) || Number(current.MAX_Access_num) || 32,
        ),
      );
      await routerPost({
        goformId: "SET_WIFI_SSID1_SETTINGS",
        ssid,
        broadcastSsidEnabled: String(current.HideSSID || "0"),
        MAX_Access_num: String(maxDevices),
        security_mode: "WPA2PSK",
        cipher: "1",
        NoForwarding: String(current.NoForwarding || "0"),
        qrcode_display_switch: "0",
        security_shared_mode: "1",
        passphrase: base64(passphrase),
      });
      const coverage = ["short_mode", "medium_mode", "long_mode"].includes(
        text(payload.coverage),
      )
        ? text(payload.coverage)
        : String(current.wifi_coverage || "short_mode");
      await routerPost({
        goformId: "SET_WIFI_COVERAGE",
        wifi_coverage: coverage,
      });
    } else if (action === "wifi_advanced_update") {
      const mode = ["0", "1", "2", "3", "4"].includes(text(payload.mode))
        ? text(payload.mode)
        : "4";
      const channel = Math.trunc(Number(payload.channel) || 0),
        bandwidth = ["0", "1", "2"].includes(text(payload.bandwidth))
          ? text(payload.bandwidth)
          : "1";
      const rate = String(Math.max(0, Math.min(65, Number(payload.rate) || 0))),
      ...
      await routerPost({
        ...
        MAX_Access_num: String(
          Math.max(1, Math.min(32, Number(payload.maxDevices) || 32)),
        ),
        m_MAX_Access_num: "0",
      ...
      const current = await routerGet(
        "Current_index,ppp_passwd_ui,ppp_passwd,pdp_type_ui,pdp_type",
      );
      const accessPoint = text(payload.accessPoint, 100);
      if (!accessPoint) throw new Error("APN is required");
      const index =
        text(payload.index, 4) || String(current.Current_index || "0");
      const authMode = ["none", "pap", "chap"].includes(
      ...
      await routerPost({
        ...
        index,
        wan_apn: accessPoint,
        ppp_auth_mode: authMode,
        ppp_username: text(payload.username, 64),
        ppp_passwd:
          text(payload.apnPassword, 64) ||
          String(current.ppp_passwd_ui || current.ppp_passwd || ""),
        dns_mode: "auto",
        prefer_dns_manual: "",
        standby_dns_manual: "",
      });
    ...
    const message =
      error instanceof Error ? error.message : "Router action failed";
    return Response.json(
